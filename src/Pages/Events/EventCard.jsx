...
const EventCard = ({ event, currentUser, onJoin }) => {
  ...
  const hasJoined = joinedUsers.includes(currentUser?.email);

  return (
    <div className="bg-base-200 p-6 rounded-xl shadow">
      <h2 className="text-xl font-bold mb-2 text-primary">{title}</h2>
      <p className="text-sm mb-1"><strong>Posted by:</strong> {name}</p>
      <p className="text-sm mb-1"><strong>Date:</strong> {format(new Date(date), 'PPP')} at {time}</p>
      <p className="text-sm mb-1"><strong>Location:</strong> {location}</p>
      <p className="text-sm mb-3">{description}</p>
      <p className="text-sm mb-2"><strong>Attendees:</strong> {attendeeCount}</p>
      <button
